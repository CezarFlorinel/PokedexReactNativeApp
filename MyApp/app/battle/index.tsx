...
type GridItem = { id: number; name: string };
...
export default function BattleScreen() {
  ...
  const grid = mapPagesToBasics(data) as GridItem[];

  const [opponent, setOpponent] = useState<GridItem | null>(null);
  ...
  const performAttack = (from: "me" | "opp", hits: number) =>
    new Promise<void>((resolve) => {
      ...
      const doOne = () => {
        ...
        Animated.parallel([runShake(attackerShake), runSlash(fromLeft)]).start(() => {
          runFlash(defenderFlash).start(() => {
            i += 1;
            if (i < hits) setTimeout(doOne, 120);
            else resolve();
          });
        });
      };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {/* left: YOU (back view) with shake + hit-flash */}
        <Animated.Image
        ...
        {/* right: OPPONENT (or Poké Ball) with shake + hit-flash */}
        {opponent ? (
          <Animated.Image
        ...
        {/* Slash projectile overlay (absolute) */}
        {arenaWidth > 0 && (
          <Animated.View
            style={[
              styles.slash,
        ...
        {/* RESULT OVERLAY (covers arena when fight ends) */}
        {result && (
          <View
            style={[
              StyleSheet.absoluteFillObject,
              styles.resultOverlay,
              { backgroundColor: result === "win" ? "rgba(30,158,94,0.85)" : "rgba(229,57,53,0.85)" },
            ]}
            pointerEvents="auto"
          >
            ...
            <Pressable onPress={resetBattle} style={styles.continueBtn} accessibilityLabel="Continue">
...
const styles = StyleSheet.create({
  ...
  mySprite: { width: 120, height: 120, transform: [{ scaleX: 1 }] },
  ...
  slash: {
    position: "absolute",
    bottom: 40, // roughly mid-line between sprites
    width: 56,
    height: 56,
  },

  // RESULT overlay over arena
  resultOverlay: {
    alignItems: "center",
